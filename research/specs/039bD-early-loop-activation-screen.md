# Spec 039bD — early loop activation + penalized_tanh middle layers

**Slug:** `early-loop-activation-penalized-tanh-screen`
**Created:** 2026-04-25
**Status:** READY
...
1. `ENABLE_LOOPING_AT=0.175` — activates looping at 17.5% of wallclock
   (~3.5 min into a 20-min run) instead of 35% (~7 min). The looped middle
   layers get significantly more training time in their recurrent regime.
2. `MLP_MIDDLE_ACTIVATION=penalized_tanh` on layers 3/4/5 — the same
   activation used in 039bA, which showed neutral pre-quant EMA loss but a
   quantization penalty. With earlier loop activation the middle layers may
   learn a more quantization-friendly representation.
...
- branch: `exp/039b-loop-band-activation-screen`
- commit: `5bbf12f`
- reference result (4×H100, ENABLE_LOOPING_AT=0.35): pre-quant EMA val_bpb=1.07410
...
- add: `MLP_OUTER_ACTIVATION=leaky_relu_square`
...
Same as 039 baseline, with one change:

```bash
ENABLE_LOOPING_AT=0.175
```

Full env (all other values unchanged from 039 baseline):
...
env \
  ...
  MLP_MIDDLE_LAYERS=3,4,5 \
  NEGATIVE_SLOPE=0.5 \
  RUN_ID="039bD-early-loop-penalized-tanh" \
  torchrun --standalone --nproc_per_node=4 train_gpt.py \
  >> /workspace/runs/039-neg-slope-screen-on-1797-base/039bD/train.log 2>&1
```

## Compare against

- 039 baseline: `ENABLE_LOOPING_AT=0.35`, no middle activation, pre-quant EMA val_bpb=1.07410, quant val_bpb=1.07410
- 039bA: `ENABLE_LOOPING_AT=0.35` + penalized_tanh mid-layers, pre-quant EMA val_bpb=1.07397, quant val_bpb=1.08312
- Question: does earlier loop activation fix the quantization penalty seen in 039bA?

## Acceptance

Interesting if pre-quant EMA val_bpb < 1.0740 (baseline) at matched steps.
