# Spec 041I — loop layers 4-5, NUM_LOOPS=2, frac=0.25

**Slug:** `041I-loop45-frac025-screen`
**Created:** 2026-04-26
**Status:** READY
...
Sibling of 041H (frac=0.35). Same N=15 config (layers 4-5, NL=2) but earlier loop
activation. Earlier activation = more loop-phase seconds = more loop steps.

- No-loop phase: 0.25 × 1200 × 5.495 = **~1648 steps** (activates early)
- Loop phase: 0.75 × 1200 × 4.167 = **~3750 loop steps**
- Total: **~5399 steps**
- Predicted val_bpb: 1.06514 - (3750-2867)×2.2e-6 = **~1.06320**

Risk: loop activates at step ~1648 — model has seen less training signal before
recurrence kicks in. May degrade quality per loop step relative to 041H.
...
export RUN_ID="041I-loop45-frac025"

mkdir -p /workspace/runs/041I-loop45-frac025-screen
...
torchrun --standalone --nproc_per_node=4 \
  ...
  >> /workspace/runs/041I-loop45-frac025-screen/train.log 2>&1
```
...
~1.06320. If early-activation penalty exists, will land above this. Compare directly
to 041H (frac=0.35, same N) to isolate the activation-timing effect.
