# Spec 041B — original loop (layers 3-5), NUM_LOOPS=1, frac=0.59

**Slug:** `041B-original-loop-1pass-late-activation-screen`
**Created:** 2026-04-25
**Status:** READY
...
**full layer range (3-5) but shallow depth (1 extra pass)** — 14 effective
passes vs baseline's 17, giving faster loop throughput.

Two changes from baseline:
1. **NUM_LOOPS=1**: 1 extra pass on layers 3-5 (14 total passes, vs 17 baseline).
   Loop throughput ~3385K tok/s (~4.304 steps/s), ~6% faster than baseline loop.
2. **ENABLE_LOOPING_AT=0.59**: activate at 708s so total steps land at ~6000.

**Expected trajectory:**
- Phase 1 (0–708s): ~5.477 steps/s → ~3878 steps
- Phase 2 (708–1200s): ~4.304 steps/s → ~2117 recurrent steps
- Total: ~5995 steps

The activation spike will be smaller than baseline (1 pass vs 2). RECUR_ALPHA_ENABLED=0
keeps carry neutral (established by 039bK/bL).
...
LOOP_START=3
LOOP_END=5
ENABLE_LOOPING_AT=0.59
RECUR_ALPHA_ENABLED=0
```

`RECUR_ALPHA_ENABLED=0` keeps carry neutral per 039bK/bL finding.
...
export NUM_LOOPS=1 LOOP_START=3 LOOP_END=5 ENABLE_LOOPING_AT=0.59
...
export RUN_ID="041B-original-loop-1pass-late-activation"

mkdir -p /workspace/runs/041B-original-loop-1pass-late-activation-screen
...
torchrun --standalone --nproc_per_node=4 \
  ...
  >> /workspace/runs/041B-original-loop-1pass-late-activation-screen/train.log 2>&1
```

## What to watch

- **Throughput Phase 1 (0–708s):** ~4308K tok/s
- **Loop activation:** around step ~3878, log shows `layer_loop:enabled`
- **Throughput Phase 2:** ~3385K tok/s (1-pass loop, faster than baseline's 3200K)
- **Step at wallclock cap:** ~5995 steps
- **Activation spike:** smaller than baseline (1 pass vs 2)
