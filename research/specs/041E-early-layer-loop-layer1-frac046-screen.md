# Spec 041E — early layer loop (layer 1 only), NUM_LOOPS=2, frac=0.46

**Slug:** `041E-early-layer-loop-layer1-frac046-screen`
**Created:** 2026-04-26
**Status:** READY
...
041D tests recurrence at a late layer (layer 5). This spec is an identical setup
but with the loop placed at an early layer (layer 1) — same N=13, same frac=0.46,
same ~3071 predicted loop steps.

The question is purely positional: does it matter *where* in the network the
recurrence happens? All prior 041/039b experiments looped late layers (3-5).
Early-layer recurrence is unexplored. Two competing priors:

- **Early layers may benefit more:** they build fundamental token representations;
  iterating there could produce richer features for the rest of the network.
- **Late layers may benefit more:** they're closest to the output and prediction
  loss; recurrence there directly refines the final representation.

If 041D and 041E land similarly → position doesn't matter, only depth and step count.
If 041E wins → early recurrence is a better inductive bias for this architecture.
If 041D wins → late recurrence remains the right choice.

## Throughput (identical to 041D)

- N = 11 + 2×1 = 13 passes
- step_time = 22.5 + 13×14.5 = 211ms → 4.739 steps/s
- Loop phase (648s at frac=0.46): ~3071 loop steps
- Predicted steps: ~6104
...
RECUR_ALPHA_ENABLED=0 required: recur_alpha/recur_beta calibrated for layers 3-5;
shape mismatch with LOOP_START=1. Carry is neutral (established by 039bK/bL).
...
export NUM_LOOPS=2 LOOP_START=1 LOOP_END=1 ENABLE_LOOPING_AT=0.46
...
export RUN_ID="041E-early-layer-loop-layer1-frac046"

mkdir -p /workspace/runs/041E-early-layer-loop-layer1-frac046-screen
...
torchrun --standalone --nproc_per_node=4 \
  ...
  >> /workspace/runs/041E-early-layer-loop-layer1-frac046-screen/train.log 2>&1
```

## What to watch

- **Loop activation:** step ~3033, log shows `layer_loop:enabled encoder:[0, 1, 1] decoder:[1, 1, 2, 3, ...]`
- **Throughput Phase 2:** ~3727K tok/s (identical to 041D)
- **Step at wallclock cap:** ~6100 steps
...
Same throughput and loop steps as 041D (~3071). If early vs late position doesn't
matter: expect ~1.06462. If early-layer recurrence is better: below that.
If worse: above 1.06514 (baseline).
