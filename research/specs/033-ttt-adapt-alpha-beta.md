...
One run is enough for the first answer because the direct comparison target already exists in `028B`.
...
- `diagnostic pre-quantization` bpb differs materially from the expected float checkpoint → halt
