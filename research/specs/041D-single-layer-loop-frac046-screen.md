# Spec 041D — single loop layer (layer 5 only), NUM_LOOPS=2, frac=0.46

**Slug:** `041D-single-layer-loop-frac046-screen`
**Created:** 2026-04-26
**Status:** READY
...
041A (layers 4-5, NUM_LOOPS=2, frac=0.46) nearly matched baseline at 5722 steps
with val_bpb 1.06545 (+0.00031). The empirical relationship shows ~2.2e-6 val_bpb
improvement per loop step. 041A had 2692 loop steps; baseline had 2867.

The bottleneck: with N=15 passes (2 loop layers × 2 extra passes + 11 base),
the loop phase runs at 240ms/step → only 2692 loop steps in 648s of loop time.

**This spec reduces to 1 loop layer (layer 5 only), keeping NUM_LOOPS=2:**
- N = 11 + 2×1 = 13 passes
- step_time = 22.5 + 13×14.5 = 211ms → 4.739 steps/s
- Loop phase (648s at frac=0.46): 3071 loop steps (+379 vs 041A)
- Predicted val_bpb: 1.06545 - 379×2.2e-6 = **~1.06462** (below baseline 1.06514)

The key prior: NUM_LOOPS=2 depth matters far more than loop width. 041A vs
baseline showed 2-layer vs 3-layer loop barely changes val_bpb. The bet here
is that 1 layer × 2 extra passes still provides meaningful recurrence while
buying 379 extra loop steps via faster throughput.
...
One fewer loop layer vs 041A (drop layer 4). RECUR_ALPHA_ENABLED=0 required:
recur_alpha/recur_beta calibrated for 3 loop layers; shape mismatch with 1
loop layer. Carry is neutral (established by 039bK/bL).
...
export NUM_LOOPS=2 LOOP_START=5 LOOP_END=5 ENABLE_LOOPING_AT=0.46
...
export RUN_ID="041D-single-layer-loop-frac046"

mkdir -p /workspace/runs/041D-single-layer-loop-frac046-screen
...
torchrun --standalone --nproc_per_node=4 \
  ...
  >> /workspace/runs/041D-single-layer-loop-frac046-screen/train.log 2>&1
```

## What to watch

- **Loop activation:** step ~3033, log shows `layer_loop:enabled`
- **Throughput Phase 2:** ~3727K tok/s (vs 041A's ~3277K — visibly faster)
- **Step at wallclock cap:** ~6100 steps
- **Activation spike:** should be smaller than 041A (only 1 layer shocked)

## Acceptance

Baseline (039b): pre-quant EMA val_bpb **1.06514**

- **Win**: pre-quant EMA val_bpb < **1.0641** — single-layer loop + more steps works
- **Noise zone**: 1.0641–1.0670 — run second seed
- **Kill**: pre-quant EMA val_bpb ≥ **1.0670** — 1 loop layer insufficient depth

## Prediction

Expected ~1.06462 based on 2.2e-6 val_bpb/loop step rate and 3071 loop steps.
