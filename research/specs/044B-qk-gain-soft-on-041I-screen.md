# Spec 044B — QK_GAIN softening combined with 041I's loop config

**Slug:** `044B-qk-gain-soft-on-041I-screen`
**Created:** 2026-04-27
...
**Links to:** `research/ideas/per-layer-qk-gain.md`,
              041I result (4-5, NL=2, frac=0.25 → EMA 1.06594)

## Hypothesis

Two independent levers:
1. **QK softening** (`QK_GAIN_INIT=2.5`) — different lever class entirely
   (attention init), per PR #1648 cross-seed evidence.
2. **041I config** (loop 4-5, NL=2, frac=0.25) — best simple loop variant
   so far, EMA 1.06594 (+0.00080 vs baseline).

041I and QK init operate on completely different mechanisms (loop architecture
vs attention sharpness). They should NOT have the negative-interaction
problem we hit with 041L (NL=3 + early activation, both pushing the loop
direction).

If the levers are even partially additive:
- 041I alone: −0.00080 vs baseline (closer to baseline by 0.00080)
- QK softening contribution (predicted): −0.001 to −0.003
- Combined: ~1.0640–1.0650 → potentially **win zone**
...
- If 044A wins and 044B doesn't beat it → QK doesn't stack with 041I (info)
...
LOOP_START=4                # was 3 (041I shrunk band)
LOOP_END=5                  # unchanged
NUM_LOOPS=2                 # unchanged (041I keeps NL=2)
ENABLE_LOOPING_AT=0.25      # was 0.35 (041I early activation)
```
...
# *** 041I shrunk-band early-activation ***
export NUM_LOOPS=2 LOOP_START=4 LOOP_END=5 ENABLE_LOOPING_AT=0.25
...
export RUN_ID="044B-qk-gain-soft-on-041I"

pip install brotli --break-system-packages -q

mkdir -p /workspace/runs/044B-qk-gain-soft-on-041I-screen
...
torchrun --standalone --nproc_per_node=4 \
  ...
  >> /workspace/runs/044B-qk-gain-soft-on-041I-screen/train.log 2>&1
```

## What to watch

- **Loop activation:** `layer_loop:enabled step:N frac:0.250` (early on shrunk band 4-5)
- **Final step count:** expect similar to 041I (~5374 steps)
...
- **Win**: < **1.0641**
- **Noise zone**: 1.0641–1.0670 (smaller-than-additive; check vs 044A and 041I to attribute)
- **Kill**: ≥ **1.0670** (negative interaction or QK kills the loop benefit)

## Predicted outcome

Starting from 041I (1.06594) and adding QK softening (predicted −0.001 to −0.003):
- Optimistic: **1.0635** (clear strong win, levers stack well)
- Realistic: **1.0648** (low noise zone, small additive effect)
- Pessimistic: **1.0655** (essentially same as 041I; QK doesn't help on this combo)

## Decision matrix (after both 044A and 044B return)

| 044A (QK alone) | 044B (QK + 041I) | Interpretation |
|---|---|---|
| Win | Win | QK lever real AND stacks → submission candidate, run second seed |
| Win | Noise | QK works alone but shrunk-band kills its benefit |
