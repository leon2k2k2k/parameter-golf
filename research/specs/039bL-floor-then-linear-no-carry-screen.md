# Spec 039bL — floor_then_linear + RECUR_ALPHA_ENABLED=0

**Slug:** `floor-then-linear-no-carry-screen`
**Created:** 2026-04-25
**Status:** READY
**Branch:** `exp/039b-loop-band-activation-screen`
**Commit:** `0516020`
**Links to:** `research/specs/039bJ-floor-then-linear-screen.md`, `research/specs/039bK-no-frozen-carry-screen.md`

## Hypothesis

039bJ (floor_then_linear) ended ~flat vs baseline. 039bK showed frozen carry has
minimal effect pre-loop. This arm combines both: floor_then_linear LR schedule
with no frozen carry scaling. If the frozen carry was hurting convergence in the
second half, removing it while keeping the gentler LR shape might help.

## Config diff

Two changes from the 039 baseline:
...
export RUN_ID="039bL-floor-then-linear-no-carry"

mkdir -p /workspace/runs/039-neg-slope-screen-on-1797-base/039bL
...
torchrun --standalone --nproc_per_node=4 \
  ...
  >> /workspace/runs/039-neg-slope-screen-on-1797-base/039bL/train.log 2>&1
```
