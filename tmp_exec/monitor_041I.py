#!/usr/bin/env python3
"""Monitor 041I vs baseline, 041A, 041J. Posts to Discord each tick."""
...
LOG_041J = "/workspace/runs/041J-loop45-frac030-screen/train.log"
LOG_041I = "/workspace/runs/041I-loop45-frac025-screen/train.log"
...
def main():
    ...
    i_lines = ssh_lines(f"grep 'train_loss' {LOG_041I} | tail -8")
    i_data   = parse_log(i_lines)
    activated, stopping, final_ema, loop_step = get_status(LOG_041I)
    pod_lines = subprocess.run("runpodctl pod list", shell=True, capture_output=True, text=True).stdout

    if not i_data:
        print(f"● 041I — {now} | no step data yet (still compiling)")
    else:
        last_step = max(i_data)
        last_loss, last_time, last_toks = i_data[last_step]
        loop_tag = f"loop@{loop_step}" if activated else "no-loop"
        print(f"● 041I — step {last_step} | loss: {last_loss:.4f} | {last_time} | {last_toks//1000}K tok/s | {loop_tag}")
        print()

        steps = sorted(i_data.keys())
        ...
        ref_041j = parse_log(ssh_lines(f"grep -E '{step_filter}' {LOG_041J}"))

        col_w = 10
        print(f"  {'Step':>6}  {'Baseline':>{col_w}}  {'041A':>{col_w}}  {'041J':>{col_w}}  {'041I':>{col_w}}")
        print("  " + "-"*6 + "  " + ("-"*col_w + "  ")*4)
        for s in steps:
            b = f"{ref_base[s][0]:.4f}" if s in ref_base else "    —   "
            a = f"{ref_041a[s][0]:.4f}" if s in ref_041a else "    —   "
            j = f"{ref_041j[s][0]:.4f}" if s in ref_041j else "    —   "
            i = f"{i_data[s][0]:.4f}"
            print(f"  {s:>6}  {b:>{col_w}}  {a:>{col_w}}  {j:>{col_w}}  {i:>{col_w}}")
        print()
