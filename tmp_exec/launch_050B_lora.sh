...
# ── 4. Config — 050 base + AC-fix + LoRA rank-2 ──────────────────────────
...
export LOOP_FFN_LORA_RANK=2     # per-pass rank-2 delta on MLP up/down
...
# 050B has two new graph variants at loop activation:
#   - block.forward with lora_delta_up=None (pre-loop)
#   - block.forward with lora_delta_up=<tensor> (post-loop)
...
STASH="/workspace/.inductor_cache_${SHA}_050B"

if [ ! -d "$STASH" ]; then
  echo "[launch] No stash found — running smoke/prewarm (10 min) to compile graphs..."
  SMOKE_LOG="${RUNDIR}/smoke.log"
  MAX_WALLCLOCK_SECONDS=600 PREQUANT_ONLY=1 RUN_ID="050B-smoke" \
...
echo "[launch] Starting screen — per-pass FFN LoRA rank-2 on 050 base + AC-fix"
