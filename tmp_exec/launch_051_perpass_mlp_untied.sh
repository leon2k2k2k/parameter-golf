#!/bin/bash
# Spec 051 — PPM-D: Per-Pass MLP Distinct (enc/dec bank split on loop layers 3-5)
# 6e6dd1e: enc visits use mlp_up/down_bank[i][:h2,:], dec uses [h2:,:].
# Same total params (mlp_mult=4 split into 2×2 halves), zero overhead.
# Builds on 050A AC-fix env (recip + iter_embeds), no LoRA, no AdaLN.
# Accept: pre-quant EMA bpb <= 1.064. Kill: > 1.068.
set -euo pipefail

SHA="6e6dd1e"
ARM="051-perpass-mlp-untied"
...
# ── 4. Config — matches 050A (AC-fix), code-only diff for the bank split ──
...
# Loop — AC-fix levers (matches 050A exactly)
...
export RUN_ID="051-perpass-mlp-untied"

echo "[launch] Starting 20-min 051 screen — per-pass MLP bank split (PPM-D)."
echo "[launch] Accept: pre-quant EMA bpb <= 1.064 vs 050A 1.06484 | Kill: > 1.068"
torchrun --standalone --nproc_per_node=4 \
  "${WORKTREE}/${TRAIN_SCRIPT}" \
  >> "${RUNDIR}/train.log" 2>&1

echo "[launch] 051 perpass-mlp-untied screen done."
