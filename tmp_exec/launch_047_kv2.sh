#!/bin/bash
# Spec 047 — KV group-of-4 screen: NUM_KV_HEADS=2 on AC-fix base (e021255)
#
# Only change from AC-fix (045 armD): NUM_KV_HEADS=4 → 2
# kv_bank: [22, 256, 512] → [22, 128, 512] — saves ~1.08M params (~600-700KB compressed)
# Goal: create headroom for 046G-tightest DCLIP (ATTN=11.5/MLP=10.5/EMB=13.5)
#
# Inductor: new KV=2 kernels needed. Seed from 7c6ac51 stash (KV=4, AC-fix base)
# — reuses MLP/embed/norm kernels, recompiles only K/V attention kernels.
# Expect ~12-15 min prewarm (vs 25 min cold), then 20 min training.
#
# Compare to AC-fix baseline: pre-quant 1.06479, post-quant 1.07387
# Accept: pre-quant ≤ 1.06679 (+0.002). Kill: > 1.06979 (+0.005)
set -euo pipefail

SHA="e021255"
ARM="047-kv2"
RUNDIR="/workspace/runs/047-kv-group4-screen"
...
export VOCAB_SIZE=8192 NUM_LAYERS=11 XSA_LAST_N=11 MODEL_DIM=512 NUM_KV_HEADS=2 NUM_HEADS=8
...
# ── 5. Inductor cache — seed from 7c6ac51 (KV=4 AC-fix base) ─────────────
# Non-attention kernels (MLP, embed, norm) reuse. K/V attention kernels recompile.
# Expect ~12-15 min prewarm vs 25 min cold.
export TORCHINDUCTOR_CACHE_DIR=/tmp/inductor_cache
mkdir -p /tmp/inductor_cache
STASH="/workspace/.inductor_cache_${SHA}_kv2"

if [ ! -d "$STASH" ]; then
  echo "[launch] No KV=2 stash — running inline prewarm (seed from 7c6ac51)..."
  if [ -d /workspace/.inductor_cache_7c6ac51 ]; then
    ...
    echo "[launch] WARNING: 7c6ac51 stash not found — cold compile (~25 min)"
  fi
  PREWARM_LOG="${RUNDIR}/prewarm.log"
  MAX_WALLCLOCK_SECONDS=300 TRAINING_ONLY_SCREEN=1 TTT_ENABLED=0 RUN_ID="047-kv2-prewarm" \
  ...
  echo "[launch] KV=2 cache restored: $(du -sh /tmp/inductor_cache | cut -f1)"
fi

# ── 6. Train ──────────────────────────────────────────────────────────────
export MAX_WALLCLOCK_SECONDS=1200 TRAINING_ONLY_SCREEN=1 TTT_ENABLED=0
export RUN_ID="047-kv2"

echo "[launch] starting torchrun — worktree: ${WORKTREE}"
echo "[launch] gate: pre-quant bpb ≤ 1.06679 (accept) | > 1.06979 (kill)"
