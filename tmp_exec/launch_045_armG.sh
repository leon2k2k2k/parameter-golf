#!/bin/bash
# Spec 045 — Arm G: LOOP_ITER_EMBEDS=1 LOOP_SCALE_INIT=recip LOOP_LR_SCALE=recip
# AC-fix + gradient-side 1/L: gradient hooks on looped bank rows and block scalars
# scale by 1/num_passes, compensating for L× gradient accumulation from weight tying.
# Compare to AC-fix result to isolate Lever G contribution.
#
# Pre-requisite: fc54262 cache must be stashed on the volume (stage 1 sufficient).
...
SHA="7c6ac51"
ARM="armG"
RUNDIR="/workspace/runs/045-loop-layer-improvements/armG"
