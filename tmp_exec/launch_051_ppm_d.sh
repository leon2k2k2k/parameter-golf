...
# ── 4. Config (shared between prewarm + real run) ─────────────────────────
...
CACHE_SIZE_AFTER=$(du -sh /tmp/inductor_cache | cut -f1)
TOK=$(grep "^100/20000 train_loss" "${RUNDIR}/prewarm.log" | tail -1 | grep -o 'tok/s: [0-9]*' | grep -o '[0-9]*' || true)
echo "[launch] Phase 0 done. Cache: ${CACHE_SIZE_BEFORE} → ${CACHE_SIZE_AFTER}. tok/s@100: ${TOK:-unknown}"

# ── Phase 1: real 20-min screen ──────────────────────────────────────────
# Kernels are now in /tmp/inductor_cache — no mid-run recompile at loop activation.
...
export RUN_ID="051-ppm-d"
torchrun --standalone --nproc_per_node=4 \
  "${WORKTREE}/${TRAIN_SCRIPT}" \
  >> "${RUNDIR}/train.log" 2>&1

echo "[launch] 051 PPM-D screen done."
