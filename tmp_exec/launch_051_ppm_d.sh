...
# Loop — AC-fix levers (same as 050A; PPM-D is code-only, no new env vars)
...
echo "[launch] Starting screen — PPM-D enc/dec MLP bank split on loop layers 3-5"
echo "[launch] Accept: pre-quant EMA bpb <= 1.064 | Kill: > 1.068"
echo "[launch] Watch: pre-loop step time ~= 050A; post-loop may be faster (half-width MLP)"
