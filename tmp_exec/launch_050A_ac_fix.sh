#!/bin/bash
# Spec 050A — AC-fix on 1797 baseline: recip-init + iter-embeds + re-warmup
# LOOP_SCALE_INIT=recip + LOOP_ITER_EMBEDS=1 on 050 base.
...
SHA="62a0c0e"
ARM="050A-ac-fix"
RUNDIR="/workspace/runs/050A-ac-fix-screen"
...
# ── 4. Config — 050 base + LOOP_SCALE_INIT=recip + LOOP_ITER_EMBEDS=1 ────
...
export LOOP_SCALE_INIT=recip    # init attn/mlp scales to 1/(num_loops+1)
export LOOP_ITER_EMBEDS=1       # per-pass embedding injection at loop entry
...
export RUN_ID="050A-ac-fix"

echo "[launch] Starting screen — AC-fix levers on 050 base"
echo "[launch] Accept: pre-quant EMA bpb <= 1.067 | Kill: > 1.072"
torchrun --standalone --nproc_per_node=4 \
  "${WORKTREE}/${TRAIN_SCRIPT}" \
  >> "${RUNDIR}/train.log" 2>&1

echo "[launch] 050A ac-fix screen done."
