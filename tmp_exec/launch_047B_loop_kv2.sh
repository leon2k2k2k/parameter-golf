#!/bin/bash
# Spec 047B — Loop-layer KV shrink: LOOP_LAYER_NUM_KV_HEADS=2 on loop layers [3,4,5]
#
# Code change: loop_kv_bank [6, 128, 512] replaces kv_bank for layers 3,4,5.
# kv_bank unchanged at [22, 256, 512] (loop rows exist, zero grad, unused).
# 02977b7 — GPTQ now wired through serialize/deserialize for loop_kv_bank.
# Full pipeline (train + GPTQ + post-quant val); TTT off for speed.
#
...
export LOOP_LAYER_NUM_KV_HEADS=2
...
# ── 5. Inductor cache — new graph (loop_kv_bank is new tensor in compute graph) ──
# Seed from e021255 stash (AC-fix, KV=4) to reuse MLP/embed/norm kernels.
# Attention kernels for loop layers will recompile (different kv_bank shape).
# Expect ~15 min prewarm.
export TORCHINDUCTOR_CACHE_DIR=/tmp/inductor_cache
mkdir -p /tmp/inductor_cache
STASH="/workspace/.inductor_cache_${SHA}_loop_kv2"

if [ ! -d "$STASH" ]; then
  echo "[launch] No stash — running inline prewarm (~15 min)..."
  for SEED_SHA in e021255 7c6ac51; do
    if [ -d "/workspace/.inductor_cache_${SEED_SHA}" ]; then
      rsync -a "/workspace/.inductor_cache_${SEED_SHA}/" /tmp/inductor_cache/
      echo "[launch] seeded from ${SEED_SHA}: $(du -sh /tmp/inductor_cache | cut -f1)"
      break
    fi
  done
  PREWARM_LOG="${RUNDIR}/prewarm.log"
  MAX_WALLCLOCK_SECONDS=300 TRAINING_ONLY_SCREEN=1 TTT_ENABLED=0 RUN_ID="047B-prewarm" \
...
# ── 6. Train + full GPTQ pipeline (TTT off) ───────────────────────────────
export MAX_WALLCLOCK_SECONDS=1200 TTT_ENABLED=0
unset TRAINING_ONLY_SCREEN
export RUN_ID="047B-loop-kv2"
...
mv "${WORKTREE}/final_model.pt" "${WORKTREE}/final_model.int6.ptz" "$RUNDIR/" 2>/dev/null || true
mv /workspace/final_model.pt /workspace/final_model.int6.ptz "$RUNDIR/" 2>/dev/null || true
echo "[launch] 047B done."
