#!/bin/bash
# Spec 050D — Per-pass FFN LoRA untied (post-activation fix) on 1797 baseline
# c1d97f9: inlined MLP path (W_up·x + ΔW_up·x → activate → W_down + ΔW_down).
# Activation evaluated at h_pre = W_up·x (Kaiming nonzero) so leaky_relu²' ≠ 0;
# fixes 050B's dead-B gradient trap. Identical dynamo topology to 050C2.
# Screen only — no smoke (graph topology validated by 050C2).
# Accept: bpb <= 1.067. Kill: > 1.072.
set -euo pipefail

SHA="c1d97f9"
ARM="050D-lora-untied"
RUNDIR="/workspace/runs/050D-lora-untied-screen"
...
export RUN_ID="050D-lora-untied"

echo "[launch] Starting 20-min 050D screen — LoRA untied, B_up gradient should flow from step 1."
echo "[launch] Accept: pre-quant EMA bpb <= 1.067 | Kill: > 1.072"
torchrun --standalone --nproc_per_node=4 \
  "${WORKTREE}/${TRAIN_SCRIPT}" \
  >> "${RUNDIR}/train.log" 2>&1

echo "[launch] 050D lora-untied screen done."
