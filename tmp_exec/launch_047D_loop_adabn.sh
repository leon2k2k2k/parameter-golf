#!/bin/bash
# Spec 047D — Per-pass AdaLN (γ,β) on loop layer attn+MLP branches
# LOOP_ADABN=1, LOOP_ITER_EMBEDS=0 (isolate AdaLN vs iter_embeds)
# 18,432 params: [3, 3, 512] × 4 tensors, init transparent.
#
# Compare to AC-fix baseline: pre-quant 1.06479
...
# ── 4. Config — AC-fix base + LOOP_ADABN=1, LOOP_ITER_EMBEDS=0 ───────────
...
export LOOP_ITER_EMBEDS=0   # replaced by LOOP_ADABN
export LOOP_ADABN=1
...
STASH="/workspace/.inductor_cache_${SHA}_adabn"
...
if [ ! -d "$STASH" ]; then
  ...
  MAX_WALLCLOCK_SECONDS=300 TRAINING_ONLY_SCREEN=1 TTT_ENABLED=0 RUN_ID="047D-prewarm" \
