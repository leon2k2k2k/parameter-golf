#!/bin/bash
# Spec 045 — Arm H: LOOP_ITER_EMBEDS=1 LOOP_SCALE_INIT=recip LOOP_PER_PASS_RESID_MIX=1
# AC-fix + per-pass residual blend: each looped layer gets independent [2, dim] blend
# params per pass. Init to [1, 0] = byte-identical to baseline at step 0.
# Compare to AC-fix result to isolate Lever H contribution.
#
# Pre-requisite: fc54262 cache must be stashed with BOTH stages (stage 2 covers
# the LOOP_PER_PASS_RESID_MIX=1 graph variant — different Block.forward path).
#   bash tmp_exec/prewarm_fc54262.sh   (on a fresh pod, ~25 min — runs both stages)
...
# NOTE: Arm H uses LOOP_PER_PASS_RESID_MIX=1 which triggers the stage-2 graph variant.
# The stashed cache must include both prewarm stages (prewarm_fc54262.sh runs both).
SRC="/workspace/.inductor_cache_${SHA}"
if [ ! -d "$SRC" ]; then
  echo "[launch] ERROR: no stashed cache for ${SHA}."
  echo "[launch] Run prewarm_fc54262.sh on a fresh pod (both stages), then cache_stash.sh."
  exit 1
fi
