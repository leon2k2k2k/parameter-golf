#!/bin/bash
# Spec 050 — Baseline arm: PR #1797 + BOS-fix + BETA2=0.99 + SPARSE_ATTN_GATE_SCALE=0.5
# Control arm for 050A/B/C comparisons. No new features.
# Compare: pre-quant EMA val_bpb ~ 1.067. Kill if > 1.072.
set -euo pipefail

SHA="f5b8af8"
ARM="050-baseline"
RUNDIR="/workspace/runs/050-baseline-screen"
...
# Quantization (only used if PREQUANT_ONLY=0)
...
# TTT (only used if PREQUANT_ONLY=0)
...
export RUN_ID="050-baseline"

echo "[launch] Starting screen — pre-quant EMA bpb target ~1.067, kill > 1.072"
torchrun --standalone --nproc_per_node=4 \
  "${WORKTREE}/${TRAIN_SCRIPT}" \
  >> "${RUNDIR}/train.log" 2>&1

echo "[launch] 050 baseline screen done."
