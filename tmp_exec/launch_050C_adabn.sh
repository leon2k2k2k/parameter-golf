...
# ── 4. Config — 050 base + LOOP_ADABN=1 (AdaLN in isolation) ─────────────
...
export RUN_ID="050C-adabn"

echo "[launch] Starting screen — per-pass AdaLN in isolation vs 050 base"
echo "[launch] Accept: pre-quant EMA bpb <= 1.067 | Kill: > 1.072"
torchrun --standalone --nproc_per_node=4 \
  "${WORKTREE}/${TRAIN_SCRIPT}" \
  >> "${RUNDIR}/train.log" 2>&1

echo "[launch] 050C adabn screen done."
