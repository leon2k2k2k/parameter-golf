...
#
# ENABLE_LOOPING_AT=0.35 (baseline comparison parity). The mid-training hang is now
# fixed via _run_cu_bucket_warmup() called at loop activation point — same approach
# as 047C fix (commit f474ad5). No need to set 0.0 anymore.
#
...
echo "[launch] ENABLE_LOOPING_AT=0.35 — re-warmup at loop activation handles graph eviction"
