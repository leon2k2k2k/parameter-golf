#!/usr/bin/env python3
"""Monitor 041J vs baseline, 041A, 041H. Posts to Discord each tick."""
...
def main():
    ...
    j_lines = ssh_lines(f"grep 'train_loss' {LOG_041J} | tail -8")
    j_data   = parse_log(j_lines)
    activated, stopping, final_ema, loop_step = get_status(LOG_041J)
    pod_lines = subprocess.run("runpodctl pod list", shell=True, capture_output=True, text=True).stdout

    if not j_data:
        print(f"● 041J — {now} | no step data yet (still compiling)")
    else:
        last_step = max(j_data)
        last_loss, last_time, last_toks = j_data[last_step]
        loop_tag = f"loop@{loop_step}" if activated else "no-loop"
        print(f"● 041J — step {last_step} | loss: {last_loss:.4f} | {last_time} | {last_toks//1000}K tok/s | {loop_tag}")
        print()

        steps = sorted(j_data.keys())
        ...
        col_w = 10
        print(f"  {'Step':>6}  {'Baseline':>{col_w}}  {'041A':>{col_w}}  {'041H':>{col_w}}  {'041J':>{col_w}}")
        print("  " + "-"*6 + "  " + ("-"*col_w + "  ")*4)
        for s in steps:
            ...
            j = f"{j_data[s][0]:.4f}"
            print(f"  {s:>6}  {b:>{col_w}}  {a:>{col_w}}  {h:>{col_w}}  {j:>{col_w}}")
        print()
