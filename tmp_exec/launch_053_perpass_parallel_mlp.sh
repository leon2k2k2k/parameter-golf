...
# loop_pass_attn_temp [3,3] ones-init: per-(pass,layer) QK scale → attention sharpness
# loop_pass_par_up/down [3,3,32,512]/[3,3,512,32]: parallel dim→32→dim MLP, zeros-out-init
# Total new params: 294,921 (~216 KB quantized). Identity at init → transparent.
# Compare to 050A baseline pre-quant EMA bpb 1.06484.
# Accept: bpb <= 1.064. Kill: > 1.068.
# Branch is exp/053-perpass-parallel-mlp (off-by-one from numbering collision at 052).
set -euo pipefail

SHA="92c8085"
ARM="053-perpass-parallel-mlp"
RUNDIR="/workspace/runs/054-perpass-parallel-mlp-screen"
...
# early, compiling both pre-loop and post-loop graph variants before Phase 1.
...
echo "[launch] 053 per-pass parallel MLP screen done."
