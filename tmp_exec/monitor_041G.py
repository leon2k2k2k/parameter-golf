#!/usr/bin/env python3
"""Monitor 041G vs baseline, 041A. Posts to Discord each tick."""
...
LOG_041G = "/workspace/runs/041G-mlp-late-shrink-loop45-screen/train.log"
...
def main():
    ...
    g_lines = ssh_lines(f"grep 'train_loss' {LOG_041G} | tail -8")
    g_data   = parse_log(g_lines)
    activated, stopping, final_ema, loop_step = get_status(LOG_041G)
    pod_lines = subprocess.run("runpodctl pod list", shell=True, capture_output=True, text=True).stdout

    if not g_data:
        print(f"● 041G — {now} | no step data yet (still compiling)")
    else:
        last_step = max(g_data)
        last_loss, last_time, last_toks = g_data[last_step]
        loop_tag = f"loop@{loop_step}" if activated else "no-loop"
        print(f"● 041G — step {last_step} | loss: {last_loss:.4f} | {last_time} | {last_toks//1000}K tok/s | {loop_tag}")
        print()

        steps = sorted(g_data.keys())
        ...
        col_w = 10
        print(f"  {'Step':>6}  {'Baseline':>{col_w}}  {'041A':>{col_w}}  {'041G':>{col_w}}")
        print("  " + "-"*6 + "  " + ("-"*col_w + "  ")*3)
        for s in steps:
            b = f"{ref_base[s][0]:.4f}" if s in ref_base else "    —   "
            a = f"{ref_041a[s][0]:.4f}" if s in ref_041a else "    —   "
            g = f"{g_data[s][0]:.4f}"
            print(f"  {s:>6}  {b:>{col_w}}  {a:>{col_w}}  {g:>{col_w}}")
        print()
