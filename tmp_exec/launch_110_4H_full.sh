#!/bin/bash
# Spec 110 FULL 4×H100 research-side run — multi-stream loop body, full training.
# Mirrors launch_110_mini.sh but uses canonical wallclock/iters (1200s / 20000 iter)
# matched-FLOPs to 8H × 600s. NOT leaderboard-valid (>600s training); for research-side
# baseline-comparison Δ vs runs/060A-1855-port/seed_42_4h/ only.
#
# Usage: SEED=42 RUN_LABEL=seed_42_4h_full bash launch_110_4H_full.sh
...
bash /workspace/parameter-golf/tmp_exec/restore_cache_local.sh "$SHA" 2>/dev/null \
  && echo "[launch] cache restored" || echo "[launch] cold compile (or warm from prior smoke)"
...
# ── 4H ADAPTATIONS — matched-FLOPs to 8H×600s ─────────────────────────────
export GRAD_ACCUM_STEPS=2          # global batch unchanged at 786432; halved GPU count → 2x accum
export MAX_WALLCLOCK_SECONDS=1200  # canonical 4H wallclock
...
echo "[launch_110_4H_full] MULTI_STREAM_LOOP_ENABLED=${MULTI_STREAM_LOOP_ENABLED} MERGE_RANK=${MULTI_STREAM_MERGE_RANK}"
echo "[launch_110_4H_full] WALLCLOCK=${MAX_WALLCLOCK_SECONDS}s ITERATIONS=${ITERATIONS} (full research run)"
...
echo "=========================================================================="
echo "[launch] 4H FULL 110 / SHA=${SHA} / SEED=${SEED} / RUN_LABEL=${RUN_LABEL}"
echo "[launch] WALLCLOCK=1200s / GRAD_ACCUM=2 (4H matched-FLOPs to 8H×600s)"
echo "[launch] NOT LEADERBOARD-VALID — research-side Δ vs 060A-1855-port/seed_42_4h"
echo "=========================================================================="
...
echo "[launch] 4H 110 full done."

# ── Verify checkpoints + cap ──────────────────────────────────────────────
...
echo "=========================================================================="
grep -E "stopping_early|diagnostic.*val_bpb|val_loss:|Total submission size|TTT|quantized.*val_bpb" "${RUNDIR}/train.log" | tail -25 || true
