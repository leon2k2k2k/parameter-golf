...
def train_model(h, device, val_data):
    ...
    # Checkpoint saving at event boundaries
    ckpt_dir = os.environ.get("CKPT_DIR", "")
    ckpt_milestones: set[int] = set()
    if ckpt_dir and h.is_main_process:
        os.makedirs(ckpt_dir, exist_ok=True)
        ckpt_milestones.add(h.muon_momentum_warmup_steps)  # momentum warmup end
        # Additional user-specified milestones
        extra_ckpts = os.environ.get("CKPT_STEPS", "")
        if extra_ckpts.strip():
            for s in extra_ckpts.split(","):
                ckpt_milestones.add(int(s.strip()))
        log(f"Checkpoints will be saved at steps: {sorted(ckpt_milestones)}")
        log(f"Note: warmdown start and recurrence activation are fraction-based, checkpoints saved at those events too")

    def save_checkpoint(step_num: int, label: str):
        if not ckpt_dir or not h.is_main_process:
            return
        ckpt_path = os.path.join(ckpt_dir, f"ckpt_{label}_step{step_num}.pt")
        torch.save({
            "step": step_num,
        ...
        log(f"Checkpoint saved: {ckpt_path} ({os.path.getsize(ckpt_path) / 1e6:.1f} MB)")

    warmdown_ckpt_saved = False
    ...
    while True:
        ...
        scale = lr_mul(frac)
        # Checkpoint at warmdown start (fraction-based)
        if not warmdown_ckpt_saved and scale < 1.0:
            warmdown_ckpt_saved = True
            save_checkpoint(step, "warmdown_start")

        if h.num_loops > 0 and not base_model.looping_active and frac >= h.enable_looping_at:
            save_checkpoint(step, "pre_recurrence")
            base_model.looping_active = True
            log(
        ...
        step += 1
        if step in ckpt_milestones:
            save_checkpoint(step, "event")
    ...
    save_checkpoint(step, "final_pre_ema")
    log("ema:applying EMA weights")
    current_state = base_model.state_dict()
    avg_state = {name: t.to(dtype=current_state[name].dtype) for (name, t) in ema_state.items()}
    base_model.load_state_dict(avg_state, strict=True)
    save_checkpoint(step, "final_post_ema")
