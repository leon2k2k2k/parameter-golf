...
class Hyperparameters:
    ...
    # Gated XSA: per-(layer, head) tanh(alpha) gate on the XSA coefficient.
    # zero-init -> tanh(0)=0 -> transparent at step 0 (pure #2014 base).
    gated_xsa_enabled = bool(int(os.environ.get("GATED_XSA", "0")))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # original _xsa_efficient is unchanged at step 0. Routed to the scalar
        # AdamW group via the ndim<2 check in Optimizers.__init__.
