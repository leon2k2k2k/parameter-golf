...
def _parse_layer_list(raw: str) -> tuple[int, ...]:
    vals = []
...
def _layer_mlp_activation_specs(h):
    middle = set(_parse_layer_list(h.mlp_middle_layers))
    specs = []
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        if self.recur_alpha_enabled:
            ...
            # Spec 037: sparse-gate 8xH promotion using updated recurrent carry
            # from the sparse learnable-alpha/beta experiment, frozen at the
            # full learned float values rather than a rounded approximation.
            self.register_buffer(
                "recur_beta",
                torch.tensor([1.5610, 1.8531, 2.1320], dtype=torch.float32),
            )
            self.register_buffer(
                "recur_alpha",
                torch.tensor(
                    [[0.2314, 0.0388, 0.0347],
                     [0.1260, -0.3438, 0.0145],
                     [0.0557, 0.1934, -0.0172]],
                    dtype=torch.float32,
                ),
            )
    ...
    def lr_mul(frac):
        if h.lr_schedule_mode == "first_half_default_then_floor":
            if frac >= 0.5:
                return h.min_lr
            frac = frac / 0.5
        if h.lr_schedule_mode == "first_half_default_then_second_half_floor":
            if frac >= 0.5:
