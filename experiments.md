...
| 047 | **Spec 047 — global KV head shrink** (NUM_KV_HEADS=2 on all 11 layers) | `exp/045-loop-layer-improvements` | e021255 | 4×H100 NE-1 | killed at step 3600 — train_loss Δ +0.0365 at kill, trending up | projected +0.010–0.015 bpb pre-quant | ❌ KILL — train_loss gap jumped from +0.013 to +0.037 on loop activation, widening throughout; non-loop layers cannot tolerate KV=2. Contrast: 047B (loop-only) ended at +0.0028, global is ~5× worse. See `research/evaluations/047-kv-group4-screen.md`. |
| 047B | **Spec 047B — loop-layer KV head shrink** (LOOP_LAYER_NUM_KV_HEADS=2 on layers 3–5 only; GPTQ-wired loop_kv_bank) | `exp/047B-loop-kv-shrink` | 02977b7 | 4×H100 NE-1 | **1.06763** pre-quant / **1.07653** post-quant | +0.00284 / +0.00266 vs AC-fix 1.06479 / 1.07387 | ❌ KILL — pre-quant above accept gate (≤1.06679), size 16.30 MB over 16 MB cap (loop_kv_bank added alongside unreduced kv_bank). Positive finding: +0.0028 for 9 effective loop-attn ops is mild, consistent with ALBERT (attention tying ≈ free). 10% loop-throughput improvement real. Iterate: (1) global NUM_KV_HEADS=2 (spec 047) for real size saving; (2) fix kv_bank shrink by removing loop rows from source bank. See `research/evaluations/047B-loop-kv-shrink-screen.md`. |
